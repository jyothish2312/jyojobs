...
long counter;  //total time for which motor has dispensed the liquid so far.
...
int potval=analogRead(regpin);//value of potentiometer is read, to set the 
int timer = map(potval,0,1023,10,2500);//timer is the pump activation time set using the potentiometer connected to regpin: min time is 10 milimeter,max time is 2.5 seconds right now.
