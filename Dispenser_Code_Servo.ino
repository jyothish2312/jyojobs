...
void setup() {
  ...
  //EEPROM.get(100, potsav);
  pinMode(sensorpin, INPUT); 

 // potval = analogRead(regpin);
 // potsav= potval;
 ...
 jyoservo.attach(pumpin);
  ...
  jyoservo.write(minpos);//pump is set to it's off state
...
//STEP 1: Loop initialisation
 ...
 /*      
   potval = analogRead(regpin);
    ...
    Serial.print(potval);
    if(!(potsav+-10<=potval<= potsav+10)){
        while(true){
            potval = analogRead(regpin);
            int halat = map(potval, 0, 1023 , 0, 180);
            jyoservo.write(halat);
            potsav = potval;
          //  EEPROM.put(100, potsav);
            if(debounce(resetpin == HIGH)){
                  break;
            }
        }
